...
Shader "Custom/MirrorWater"
{
	Properties
	{
		...
		//平面反射
		[Space(10)][Header(________ Planar Reflection ________)][Space(10)]
		_ReflectionTex("Reflection Texture (Auto)", 2D) = "black" {}  // 反射贴图（由C#脚本传入）
		_ReflectionStrength("Reflection Strength", Range(0, 1)) = 0.5 // 反射强度
		_ReflectionDistortion("Reflection Distortion", Range(0, 0.1)) = 0.02  // 反射扭曲程度
		[ASEEnd]_FresnelPower("Fresnel Power", Range(0.1, 10)) = 3 // 菲涅尔指数
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				// 平面反射部分
				// 计算反射UV（使用屏幕坐标 + 法线扭曲）
				float2 reflectionUV = ase_screenPosNorm.xy;
				reflectionUV += Normals81.xy * _ReflectionDistortion;
				
				// 采样反射贴图
				// 这张贴图是C#脚本用反射相机渲染后传进来的
				float4 reflectionColor = tex2D(_ReflectionTex, reflectionUV);
				
				// 计算菲涅尔效果：视角越斜，反射越强
				float NdotV = saturate(dot(WorldNormal, WorldViewDirection));
				// 视角越斜（NdotV越小），fresnel越大，反射越强
				float fresnel = pow(1.0 - NdotV, _FresnelPower);
				
				// 混合原始颜色和反射
				float4 colorWithReflection = lerp(clampResult109, reflectionColor, fresnel * _ReflectionStrength);
				// 结束添加的平面反射部分
				
				float3 Albedo = colorWithReflection.rgb;
				// 最终输出
